...
def check_motor_position(moving_dx_id) :
    ...
    if dxl_comm_result != COMM_SUCCESS:
        ...
        print("%s" % packetHandler.getRxPacketError(dxl_error))
    print(f"{moving_dx_id} Current Position : {dxl_present_position}.")
...
def initiallize_poze(dx_id) :
    ...
    if (dx_id == 13) :
        ...
        limit_range_start_to_back = 5000
        limit_range_end_to_back = 5200

    print(check_current_poze_start, check_current_poze_end)
    print(check_current_poze_end, limit_range_start_to_back)
